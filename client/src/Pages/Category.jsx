...
const Category = () => {
  ...
  const deleteCategory = async (categoryId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/delete-category/${categoryId}?confirm=true`, {
        method: 'DELETE',
        headers: {
 ...
 {categories.length ? <div className="flex-1 p-4 ">
  ...
  <tbody className='text-center'>  
    {filetredCategory.map(category => (
      <tr key={category.id} className='border-t-8 border-b-4 border-gray-200 bg-gray-300 '>
        ...
        <td className=" p-2">
          <img src={`http://localhost:3001/uploads/${category.imagefile}`} alt={category.categoryname} className='h-10 w-16 object-cover mx-auto' />
        </td>
        ...
        <td className=" p-2 flex justify-center">
          <Link to={`/edit-category/${category.id}`} className="w-fit bg-blue-500 text-white py-1 px-2 rounded mr-2 flex items-center">
  ...
  </div>:<div className='flex justify-center items-center mx-auto text-2xl text-purple-400 font-semibold'>No Categories Available. <Link to='/add-category' className='bg-purple-500 px-5 py-1 rounded-2xl text-white font-normal'>Add</Link></div>}
</div>
  );
};
